...
import videomeeting.protocol.ptcl.client2Manager.http.CommonProtocol.{GetLiveInfoRsp, GetLiveInfoRsp4RM}
...
object MeetingActor {
  ...
  final case class GetMeetingInfo(replyTo: ActorRef[MeetInfo]) extends Command //考虑后续房间的建立不依赖ws
  ...
  private def init(
    ...
    meetingInfoOpt: Option[MeetInfo] = None
  )
    (
      implicit stashBuffer: StashBuffer[Command],
      timer: TimerScheduler[Command],
    ...
    Behaviors.receive[Command] { (ctx, msg) =>
      msg match {
        case ActorProtocol.StartRoom4Anchor(userId, `meetingId`, actor) =>
...
//            ProcessorClient.closeRoom(wholeRoomInfo.roomInfo.roomId)
          if (startTime != -1l) {
            ...
            meetingManager ! MeetingManager.DelaySeekRecord(wholeRoomInfo, totalView, roomId, startTime, liveInfoMap(Role.host)(wholeRoomInfo.roomInfo.userId).liveId)
          }
       ...
       /* wholeRoomInfo.roomInfo.rtmp match {
        ...
        wholeRoomInfo.roomInfo.rtmp match {
          case Some(v) =>
            ...
            if (startTime != -1l) {
              meetingManager ! MeetingManager.DelaySeekRecord(wholeRoomInfo, totalView, roomId, startTime, v)
            }
          case None =>
        }
//        if (wholeRoomInfo.roomInfo.rtmp.get != liveInfoMap(Role.host)(wholeRoomInfo.roomInfo.userId).liveId)
//          ProcessorClient.closeRoom(roomId)
  ...
  private def changeMode(ctx: ActorContext[MeetingActor.Command], anchorUid: Long, dispatchTo: (List[(Long, Boolean)], WsMsgRm) => Unit)(roomId: Long, liveIdList: List[String], screenLayout: Int, aiMode: Int, startTime: Long) = {
