package videomeeting.pcClient.utils

import java.io.File

import videomeeting.pcClient.common.Routes
...
object RMClient extends HttpUtil {
  ...
  def getRoomList: Future[Either[Throwable, MeetingListRsp]] = {
    val methodName = "getRoomList"
    val url = Routes.getRoomList

    getRequestSend(methodName, url, Nil, needLogRsp = false).map {
      case Right(jsonStr) =>
        decode[MeetingListRsp](jsonStr)
      case Left(error) =>
